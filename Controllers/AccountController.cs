...
namespace RoleBasedAuthorization.Controllers
{
    public class AccountController : Controller
    {
        ...
        {
            var _admin = _context.Admins.Where(s => s.Email == admin.Email).FirstOrDefault();
            if (_admin != null)
            {
                if (_admin.Password == admin.Password)
                {
                    ...
                    List<Menus> menus = _context.LinkRolesMenus.Where(s => s.RolesId == roleId).Select(s => s.Menus).ToList();
